...
function ProductImageCarousel({ product, productId, productName }) {
  ...
  const images = useMemo(() => {
    ...
    if (mainImage) imageList.push({ url: mainImage, label: "Main" });

    if (Array.isArray(product.images)) {
      product.images.forEach((img, index) => {
        const url = extractImageUrl(img);
        if (url) imageList.push({ url, label: `Image ${index + 1}` });
      });
    }
    ...
    if (Array.isArray(product.variants)) {
      product.variants.forEach((variant) => {
        ...
        const vImage = extractImageUrl(
          variant.imageUrl || variant.image || variant.imageUrlSecure || variant.img
        );
        if (vImage) imageList.push({ url: vImage, label: variant.colorName || "Variant" });
      });
    }
  ...
  const safeImages = images.length > 0 ? images : [{ url: FALLBACK_IMAGE, label: "Product" }];
  ...
  return (
    <div className="w-full">
      <div className="relative aspect-square w-full bg-[#FCFCFC] overflow-hidden flex items-center justify-center p-4">
        <Link to={`/product/${productId}`} className="absolute inset-0 flex items-center justify-center p-4">
          <img
            ...
            {safeImages.map((image, index) => (
              <button
                ...
                className={`relative shrink-0 w-9 h-9 rounded-lg overflow-hidden bg-white border transition-all ${
                  currentIndex === index ? "border-black ring-1 ring-black scale-105" : "border-neutral-200 opacity-70 hover:opacity-100"
                }`}
              >
                <img src={image.url} alt={image.label} className="w-full h-full object-contain p-0.5" loading="lazy" />
              </button>
            ))}
          </div>
        </div>
...
function Shop() {
  ...
  useEffect(() => {
    ...
    const fetchShopData = async () => {
      try {
        setLoading(true);
        const [prodSnap, catSnap, revSnap] = await Promise.all([
  ...
  const getProductRatingDetails = (productId) => {
    const prodReviews = reviews.filter((r) => String(r.productId) === String(productId));
    if (prodReviews.length === 0) return { average: "5.0", count: 0 };
    const total = prodReviews.reduce((acc, curr) => acc + Number(curr.rating || 0), 0);
    return { average: (total / prodReviews.length).toFixed(1), count: prodReviews.length };
  };
  ...
  return (
    <div className="bg-white min-h-screen pb-16">
      <Helmet>
        <title>Shop Collection | Opera Jewellery PK</title>
        <meta name="description" content="Explore our luxurious collection of fine American zircon jewellery at Opera PK." />
      </Helmet>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b border-neutral-200/60 pb-5">
          ...
          <div className="text-xs uppercase tracking-[0.2em] text-neutral-500 font-bold self-end md:self-auto bg-[#FDFBF7] px-3.5 py-1.5 rounded-md border border-[#C5A059]/20 shadow-xs">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {filteredProducts.map((product) => {
              ...
              const mainImage = extractImageUrl(product.image || product.imageUrl || product.img) || FALLBACK_IMAGE;
              const isWishlisted = isInWishlist(productId);
              const ratingData = getProductRatingDetails(productId);
              ...
              return (
                ...
                >
                  ...
                  <div className="relative">
                    <ProductImageCarousel product={product} productId={productId} productName={product.name || product.title} />
